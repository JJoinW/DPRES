...
def padding_sentence_sequences(index_sequences, scores, maxnum=512):
    X = np.empty([len(index_sequences), maxnum], dtype=np.int32)
    Y = np.empty([len(index_sequences), len(scores)], dtype=np.float32)
    mask = np.zeros([len(index_sequences), maxnum], dtype=np.int32)
    ...
    for i in range(len(index_sequences)):
        ...
        for s_i in range(len(scores)):
            Y[i][s_i] = scores[s_i][i]
    return X, mask, Y


def padding_sentence_sequences_forLLM(index_sequences, trait_num=5, maxnum=512, post_padding=True):
    X = np.empty([len(index_sequences), maxnum], dtype=np.int32)
    mask = np.zeros([len(index_sequences), maxnum], dtype=np.int32)
...
def padding_sentence_sequences_forLLM_single(index_sequences, trait_num=5, maxnum=512, post_padding=True):
    X = np.empty([len(index_sequences), trait_num, maxnum], dtype=np.int32)
    mask = np.zeros([len(index_sequences), trait_num, maxnum], dtype=np.int32)

    for i in range(len(index_sequences)):
        mul_sequence_ids = index_sequences[i]
        for idx, sequence_ids in enumerate(mul_sequence_ids):
            num = len(sequence_ids)
            if num > maxnum:
                for k in range(maxnum):
                    wid = sequence_ids[k]
                ...
                for k in range(num):
                    wid = sequence_ids[k]
                    X[i, idx, k] = wid
                X[i, idx, num:] = 1  # pad 1 for robert
                mask[i, idx, :num] = 1

    return X, mask


def normal_sequences(scores):
    Y = np.empty([len(scores[0]), len(scores)], dtype=np.float32)
...
def padding_sentence_sequences_for_gen(index_sequences, norm_score, scores, maxnum=768, post_padding=True):
    X = np.empty([len(index_sequences), maxnum], dtype=np.int32)
    Y = np.empty([len(index_sequences), 64], dtype=np.int32)
    score = np.empty([len(index_sequences), len(norm_score)], dtype=np.float32)
    mask_x = np.zeros([len(index_sequences), maxnum], dtype=np.int32)
    mask_Y = np.zeros([len(index_sequences), 64], dtype=np.int32)
    ...
    for i in range(len(index_sequences)):
        ...
        for s_i in range(len(norm_score)):
            score[i][s_i] = norm_score[s_i][i]

    for i in range(len(scores)):
        sequence_ids = scores[i]
        num = len(sequence_ids)
        if num > 128:
            for k in range(128):
                wid = sequence_ids[k]
            ...
            for k in range(num):
                wid = sequence_ids[k]
                Y[i, k] = wid
            Y[i, num:] = 1  # pad 1 for robert
            mask_Y[i, :num] = 1

    return X, mask_x, score, Y, mask_Y


def padding_sentence_sequences_single(index_sequences, scores, maxnum=512, post_padding=True):
    X = np.empty([len(index_sequences), maxnum], dtype=np.int32)
...
def normal_rescale_tointscore(scaled_scores, set_ids):
    '''
    rescale scaled scores range[0,1] to original integer scores based on  their set_ids
    :param scaled_scores: list of scaled scores range [0,1] of essays
    :param set_ids: list of corresponding set IDs of essays, integer from 1 to 8
